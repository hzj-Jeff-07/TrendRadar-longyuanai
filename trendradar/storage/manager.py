...
class StorageManager:
        ...
        if self.backend_type == "auto":
            if self.is_github_actions():
                ...
                if self._has_remote_config():
                    ...
                    logger.info("[存储管理器] GitHub Actions 环境但未配置远程存储，使用本地存储")
                    return "local"
            else:
                return "local"
    ...
    def _has_remote_config(self) -> bool:
        ...
        if not has_config:
            logger.warning("[存储管理器] 远程存储配置检查失败:")
            logger.info(f"  - bucket_name: {'已配置' if bucket_name else '未配置'}")
            logger.info(f"  - access_key_id: {'已配置' if access_key else '未配置'}")
            logger.info(f"  - secret_access_key: {'已配置' if secret_key else '未配置'}")
            logger.info(f"  - endpoint_url: {'已配置' if endpoint else '未配置'}")
    ...
    def _create_remote_backend(self) -> Optional[StorageBackend]:
        ...
        try:
            ...
            logger.warning(f"[存储管理器] 远程后端导入失败: {e}")
            logger.info("[存储管理器] 请确保已安装 boto3: pip install boto3")
            return None
        except Exception as e:
            logger.warning(f"[存储管理器] 远程后端初始化失败: {e}")
            return None
    ...
    def get_backend(self) -> StorageBackend:
        ...
        if self._backend is None:
            ...
            if resolved_type == "remote":
                ...
                if self._backend:
                    logger.info("[存储管理器] 使用远程存储后端")
                else:
                    logger.info("[存储管理器] 回退到本地存储")
                    resolved_type = "local"
            ...
            if resolved_type == "local" or self._backend is None:
                ...
                logger.info(f"[存储管理器] 使用本地存储后端 (数据目录: {self.data_dir})")
    ...
    def pull_from_remote(self) -> int:
        ...
        if not self._has_remote_config():
            logger.warning("[存储管理器] 未配置远程存储，无法拉取")
            return 0
        ...
        if self._remote_backend is None:
            logger.warning("[存储管理器] 无法创建远程后端，拉取失败")
            return 0
